...
- **Потребность («к обеспечению») берётся из сделок в статусах `SUPPLY_DEAL_STATUSES` = `CONFIRMED`, `CONTRACT`, `EXECUTION`** — это стадии, где состав уже согласован с клиентом и товар фактически обещан. Раньше по воронке состава ещё нет (в `CONFIRMED` он обязателен — см. `dealStatusRequiresItems`), позже сделка закрыта и склад ей ничего не должен.
