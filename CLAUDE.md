...
- **Поля «наша компания» в сделке пока нет** — справочник заведён под будущую автоподстановку (аукционные сделки из Tenderland, КП, отгрузки). Когда оно появится, значение подставляется в момент создания и дальше хранится снимком в сделке — как скидка, а не пересчитывается от текущей настройки.
