...
Кнопка «Обновить закупки» и ночной [scripts/tenders-cron.sh](scripts/tenders-cron.sh) дергают один и тот же [POST /api/crm/tenders/sync](app/api/crm/tenders/sync/route.js), который делает два шага: `syncTenders` (забрать новые) и `refreshTenders` (сверить те, что в работе).

Правила, которые легко сломать:

- **Точка инкремента — идентификатор, а не дата.** `searchAfterId` от максимального известного `TL\d+`: идентификатор выдаётся при добавлении закупки в Тендерлэнд и растёт монотонно. Раньше точкой служило `importedAt`, и это молча теряло данные — фильтр на их стороне работает по дате создания закупки **у них**, поэтому недобранный из-за потолка хвост отсекался на следующем же запуске.
- **Потолок — 100 закупок за ответ** (ограничение тарифа, поднимается через персонального менеджера), поэтому загрузка идёт циклом с `orderBy: tender_publishDate.asc`. Сортировка по возрастанию обязательна: при «свежие первыми» потолок отрезает самые старые записи, а дотянуться до них потом нечем — верхней границы по дате в API нет (проверено: неизвестные параметры вроде `searchBeforeCreateDate` молча игнорируются).
- **Архив не тянем.** При пустой базе берётся одна страница самых свежих (`publishDate.desc`), а не весь автопоиск: там тысячи закупок за все месяцы, это суточный лимит данных на несколько дней вперёд. Полная выгрузка — только `{ full: true }` и только админом.
- **Сверка бесплатна, добор — нет.** `Entity/GetEvents` отдаёт `lastUpdateDate` пачкой ключей за один запрос и не тратит лимит переданных данных; полные данные (`Search/Get`) добираются только по закупкам, у которых дата сдвинулась с прошлого раза. Новым записям отметка проставляется сразу при импорте (`stampEvents`), иначе первая же сверка полезла бы добирать только что забранное.
...
- **Ручная закупка помечена `Tender.source = "MANUAL"` и в курсор синхронизации не входит.** Идентификатор у неё может быть свежее всего, что забрано из автопоиска, — попав в `lastTenderId`, она сдвинула бы `searchAfterId` и молча отрезала хвост (ровно та потеря, ради которой инкремент и перевели с даты на id). Если ту же закупку позже принесёт автопоиск, пометка снимается: там она законно участвует в курсоре.
