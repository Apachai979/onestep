...
const PhoneInfo = ({ className }) => (
    <div className={clsx("flex-none -space-y-1.5", className)}>
        <p className='text-xs font-semibold text-stone-400'>info@onestep.su</p>
...
export default function Navigation() {
    ...
    const navClasses = clsx(
        ...
        {
            "top-[64px] opacity-100 z-20": !isActive,
            "top-[-400px] opacity-0 z-[-1] sm920:opacity-100 sm920:z-auto sm920:static": isActive,
        }
    )
        ...
        <>
            <ul className={navClasses}>
                <li className='mx-4 mb-8 mt-2 text-center sm920:mx-0 sm920:my-0'>
                    <Link
                        href='/catalogs'
                        aria-current={pathname === "/catalogs" ? "page" : undefined}
                        className={clsx(
                            "rounded-full px-14 py-2 text-center transition-all sm920:px-6 sm920:py-2",
                            pathname === "/catalogs"
                                ? "bg-contrast_green text-white shadow-inner shadow-gray-600/50"
                                : "bg-primary_green text-white shadow-md hover:bg-contrast_green hover:shadow-inner"
                        )}
                    >
                        Каталог
                    </Link>
                ...
                {navItems.map(link => (
                    <li key={link.label} className='mx-4 my-6 text-center sm920:mx-0 sm920:my-0'>
                        <Link
                            href={link.href}
