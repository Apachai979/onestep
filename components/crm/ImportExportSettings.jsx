...
// Порядок = рекомендованный порядок переезда: сделки ссылаются на проекты.
const ENTITIES = [
    { key: "counterparties", label: "Контрагенты", hint: "+ лист «Контакты»" },
    { key: "projects", label: "Проекты", hint: "+ лист «Позиции»" },
    { key: "deals", label: "Сделки", hint: "+ лист «Позиции», включая аукционы" },
]
    ...
    return (
        <section className='rounded-2xl border border-line bg-white p-6 shadow-sm'>
            ...
            <p className='mt-1 text-sm text-neutral-500'>
                ...
                файла. Порядок переезда: контрагенты → проекты → сделки (тогда
                восстановятся связи с контактами и проектами).
            </p>
            ...
            <div className='mt-5'>
                ...
                <p className='mt-2 text-xs text-neutral-400'>
                    Контрагенты сверяются по ИНН+КПП и названию — дубли пропускаются,
                    филиалы с разным КПП заводятся отдельно. Сделки и проекты
                    привязываются к контрагентам по ИНН/названию, менеджеры — по email
                    (иначе назначаетесь вы); дедупликации у них нет — повторная загрузка
                    того же файла создаст копии. Строки с ошибками пропускаются, а при
                    сбое импорт откатывается целиком.
                </p>
