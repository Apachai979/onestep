        ...
        <form onSubmit={handleSubmit} className='space-y-6'>
            ...
            {duplicate && (
                <div className='rounded-lg border border-yellow-300 bg-yellow-50 p-4 text-sm'>
                    ...
                    {/* Дубль на чужого дистрибьютора часто означает не два живых
                        проекта, а один забытый: работа там давно кончилась, но
                        статус никто не сменил. Рекомендацию даём сразу над
                        списком — ссылка на исходный проект должна быть под
                        рукой в момент чтения, а не после заполнения формы.
                        Пометка о дубле остаётся в новой карточке навсегда,
                        поэтому лучше разобраться до создания. */}
                    <p className='mt-1 text-yellow-800'>
                        {duplicate.requiresComment
                            ? "Этот конечный потребитель уже в работе у другого дистрибьютора. Откройте карточку проекта ниже и проверьте: если работа по нему завершена и незакрытых сделок нет — лучше перевести его в статус «Проработано, нет потребности», чем заводить дубль. Пометка о дубле останется в новом проекте навсегда."
                            : allDuplicatesClosed
