...
// «Закрыто», «Не реализована» и ARCHIVED — в архив сделки уезжают из
// CANCELLED автоматически (см. autoArchiveStaleCancelledDeals), поэтому
...
// CANCELLED, то есть осознанно. Поэтому CLOSED и не архивируется по времени —
// иначе этот запрет обходился бы простым ожиданием.
...
// «Проработано, нет потребности» — утверждение, что по проекту ничего не едет.
// Живая сделка его опровергает: товар клиенту обещан, он висит в воронке и в
// резерве отчёта обеспечения. Поэтому закрыть проект можно, только когда все
// его сделки доведены до конца — «Закрыто» или «Не реализована».
// Исключения для админа нет: это целостность данных, а не полномочия.
export const PROJECT_NO_NEED_HAS_OPEN_DEALS_ERROR =
    "По проекту есть незакрытые сделки — сначала доведите их до «Закрыто» " +
    "или переведите в «Не реализована»"

// Обратная сторона того же правила. Без неё запрет обходится с другого конца:
// закрыли проект, потом завели в нём сделку. На карточке закрытого проекта
// кнопки создания нет, но остаются прямая ссылка /crm/deals/new?fromProjectId=
// и привязка проекта к уже существующей сделке.
export const DEAL_PROJECT_NO_NEED_ERROR =
    "Проект в статусе «Проработано, нет потребности» — сначала верните его в работу"
