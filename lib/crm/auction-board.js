...
export function isOnAuctionBoard(deal, today) {
    ...
    const days = auctionDaysLeft(deal, today)
    // Отменённая закупка без срока — разбираться уже не с чем, а уйти по
    // правилу «неделя от торгов» ей нечем: без даты отсчитывать не от чего.
    // Иначе такие карточки навсегда оседали бы в «Без даты».
    if (days === null) return deal.status !== "CANCELLED"
