...
// Единственный критерий входа в «Согласовано / Позиции» — заполненный
// состав позиций: именно из него потом считается плановая потребность склада, и
// пустая сделка в этой колонке сделала бы расчёт бессмысленным. Ни цены, ни даты
// отгрузки здесь не требуем намеренно — на этой стадии они ещё меняются, а дата
// отгрузки у нас вообще становится известна накануне.
export function dealStatusRequiresItems(status) {
    return status === "CONFIRMED"
}

export const DEAL_STATUS_NEEDS_ITEMS_ERROR =
    "Статус «Согласовано / Позиции» требует заполненного состава — добавьте товарные позиции"
