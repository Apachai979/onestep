...
// У аукциона две стороны: клиент (с кем договор) и заказчик — конечный
// потребитель из закупки. Без заказчика карточка сделки неполна, поэтому поле
// обязательно. Проверка живёт на сервере, а не только в форме: мимо неё идут
// импорт из Excel и прямые PATCH.
export const DEAL_AUCTION_CUSTOMER_REQUIRED_ERROR =
    "У аукциона должен быть заказчик (конечный потребитель) — выберите контрагента"

// Требование заказчика проверяем только когда запрос сам трогает аукционные
// поля: частичный PATCH (смена статуса из карточки) не должен спотыкаться о
// старую запись, у которой заказчика нет. Возвращает текст ошибки или null.
export function dealAuctionCustomerError(data, existing = null) {
    if (data.isAuction === undefined && data.auctionCustomerId === undefined) return null
    const isAuction = data.isAuction ?? existing?.isAuction ?? false
    const customerId =
        data.auctionCustomerId !== undefined
            ? data.auctionCustomerId
            : existing?.auctionCustomerId ?? null
    if (isAuction && !customerId) return DEAL_AUCTION_CUSTOMER_REQUIRED_ERROR
    return null
}
...
// С чего сделку заводят. «Выполнение», «Ожидание» и завершённые статусы — это
// результат работы по сделке, а не точка входа: заведённая сразу закрытой
// сделка проскакивает воронку мимо всех проверок состава и попадает в историю
// как исполненная, ничего не отгрузив.
export const DEAL_CREATE_STATUSES = ["NEGOTIATION", "CONFIRMED", "CONTRACT"]

export const DEAL_CREATE_STATUS_ERROR =
    "Новую сделку можно создать только в статусе «Переговоры / КП», " +
    "«Согласовано / Позиции» или «Договор / Счёт»"
...
// CONFIRMED — клиент подтвердил, что берёт: документов ещё нет, но сделка
// почти наверняка состоится. AWAITING — сбор закрывающих документов и ожидание
// оплаты; CLOSED — сделка полностью исполнена и контроля не требует. В архив
// исполненные сделки не уезжают: это история отгрузок и денег, она остаётся на виду.
export const DEAL_STATUS_HINTS = {
    CONFIRMED: "Состав позиций согласован с клиентом",
...
// Статусы, в которых сделка ещё требует работы менеджера.
export const DEAL_OPEN_STATUSES = [
    "NEGOTIATION",
    "CONFIRMED",
    "CONTRACT",
    "EXECUTION",
...
// Брошенная сделка: в работе, но следующий шаг не запланирован — ни одной
// открытой задачи. Такие карточки тонут в колонке «Переговоры / КП» и создают
// иллюзию живой воронки, поэтому на доске их подсвечиваем.
//
// deal.tasks — открытые задачи, отфильтрованные в самом include (см. DEAL_INCLUDE
// в app/api/crm/deals/route.js). Без них признак не считается: отсутствие поля
// значит «не знаем», а не «задач нет».
export function isDealAbandoned(deal) {
    if (!DEAL_OPEN_STATUSES.includes(deal?.status)) return false
    if (!Array.isArray(deal?.tasks)) return false
    return deal.tasks.length === 0
}
...
// purchaseNumber и auctionCustomer.name (номер закупки и заказчик у аукционов).
