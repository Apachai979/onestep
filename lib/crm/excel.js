...
export function cellInt(v) {
    const n = cellNum(v)
    return n === null ? null : Math.trunc(n)
}

// «Да» / «да» / «+» / «1» → true, всё остальное → false.
export function cellBool(v) {
    const s = cellStr(v).toLowerCase()
    return s === "да" || s === "+" || s === "1" || s === "true" || s === "yes"
}

// «03.07.2026» или «03.07.2026 14:30» или Date-ячейка → Date | null
...
// Аукционные сроки задаются с точностью до минут — выгружаем со временем,
// иначе round-trip через Excel их обнуляет.
