...
const DECIMAL_FIELDS = [
    ...
    { field: "unitWeightKg", min: 0, label: "Вес единицы, кг" },
    { field: "unitVolumeM3", min: 0, label: "Объём единицы, м³" },
]

const NULLABLE_DECIMAL_FIELDS = new Set([
    "recommendedLpuPrice",
    "unitWeightKg",
    "unitVolumeM3",
])
...
export function parseProductPayload(body, { partial = false } = {}) {
    ...
    for (const { field, min, label } of DECIMAL_FIELDS) {
        ...
        if (NULLABLE_DECIMAL_FIELDS.has(field)) data[field] = value
