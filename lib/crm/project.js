...
export async function findProjectDuplicates(
    ...
    const items = found
        ...
        // Чужой дистрибьютор важнее — показываем его первым.
        .sort((a, b) => Number(a.sameDistributor) - Number(b.sameDistributor))

    return { items, hasOtherDistributor: items.some(p => !p.sameDistributor) }
}
