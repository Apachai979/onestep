...
import { formatProposalNumber, proposalNumberBase } from "./proposal-number"
import { PROPOSAL_INTRO, SELLER } from "./proposal-seller"

// Общая сборка данных для PDF коммерческого предложения.
// Используется роутами /proposal/pdf (предпросмотр, скачивание) и /proposal/send.
...
export async function buildProposalDoc(dealId, body = {}) {
    const deal = await prisma.deal.findUnique({
        ...
        include: {
            ...
            contact: true,
            // Конечный потребитель — из проекта-источника, а для сделок-аукционов
            // — это заказчик аукциона.
    ...
    const items = itemsWithProduct.map((it, i) => {
        ...
        // «Тр. уп.» — сколько коробок физически поедет, поэтому округляем
        // вверх: неполная коробка всё равно занимает место в машине.
        // Отгрузить при этом можно и некратно упаковке — в штуках заказывают
        // столько, сколько нужно, дробных коробок просто не бывает.
        const packs = packQty > 0 ? Math.ceil(qty / packQty) : null
    ...
    const endCustomerFromProject =
        deal.auctionCustomer?.name || deal.sourceProject?.endCustomer?.name || ""
    // Строка выводится, только если явно не отключена галочкой и не пуста.
    const endCustomer =
        body.showEndCustomer === false
            ? ""
            : String(
                  body.endCustomer !== undefined ? body.endCustomer : endCustomerFromProject
              ).trim()

    // Форма всегда присылает номер; фоллбэк нужен вызовам без неё.
    const number =
        String(body.number || "").trim() ||
        formatProposalNumber(proposalNumberBase(deal.id), 1)
    ...
    const docData = {
        ...
        // Покупатель — плательщик сделки, если клиент попросил оформить
        // документы на другое своё юрлицо.
        buyer: String(body.buyer || deal.payer?.name || deal.counterparty.name || "").trim(),
        endCustomer,
        ...
        intro: String(body.intro || "").trim() || PROPOSAL_INTRO,
        items,
        totals,
