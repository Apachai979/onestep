...
const s = StyleSheet.create({
    ...
    paramLabel: {
        ...
        flexShrink: 0, // лейбл не сжимаем: сжатый бокс текст не переносит,
        // а рисует поверх соседа (см. комментарий к paramValue)
    },
    paramValue: {
        // flex: 1, а не flexShrink: 1. Текст раскладывается по строкам ДО того,
        // как flex сожмёт бокс, поэтому у сжатого бокса строки остаются
        // рассчитанными по полной ширине страницы: длинное название
        // («Областное государственное автономное учреждение …») переносилось
        // не там, где надо, и вылезало за правое поле листа. flex: 1 задаёт
        // ширину заранее — перенос считается уже по ней.
        flex: 1,
