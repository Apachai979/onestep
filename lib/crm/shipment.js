...
function recipientContactName(c) {
    const fn = `${c.firstName ?? ""} ${c.lastName ?? ""}`.trim()
    return fn || c.email || c.phone || "Контакт"
}

// Разбор получателя отгрузки: выбранный контакт либо заполненные вручную поля.
// Обычная функция (не в client-компоненте) — безопасна и на сервере, и на клиенте.
export function shipmentRecipientParts(shipment) {
    if (shipment.recipientContact) {
        const c = shipment.recipientContact
        return {
            name: recipientContactName(c),
            phone: c.phone || null,
            email: c.email || null,
            wrapped: true,
        }
    ...
    return {
        ...
        phone: shipment.recipientPhone || null,
        email: shipment.recipientEmail || null,
        wrapped: false,
    }
}

export function hasShipmentRecipient(shipment) {
    const { name, phone, email } = shipmentRecipientParts(shipment)
 ...
 * deal.items — DealItem[] (с полями quantity и amount)
 * deal.shipments — Shipment[] с include items, статус SHIPPED считается отгруженным.
 * deal.discount — скидка по сделке в процентах (0..100) или null.
 *
 * Цена единицы выводится из позиции сделки: amount / quantity.
 * Все денежные значения — со скидкой: сумма позиции умножается на (1 − discount/100),
 * так же как итог сделки в карточке. Суммы до скидки лежат в *Gross.
 *
 * Возвращает {
 *   byItem: { [dealItemId]: { ordered, shipped, remaining,
 *                             unitPrice, amountOrdered, amountShipped, amountRemaining } },
 *   totalOrdered, totalShipped, totalRemaining,
 *   amountOrdered, amountShipped, amountRemaining,
 *   amountOrderedGross, discountPercent,
 *   percent (0..100, с учётом не-нулевого деления),
 *   percentAmount (0..100, доля отгруженного в деньгах),
 *   isFullyShipped
 * }
 */
export function calculateDealShipmentProgress(deal) {
    ...
    let totalShipped = 0
    let amountOrdered = 0
    let amountOrderedGross = 0
    let amountShipped = 0

    // Скидка задаётся в процентах на всю сделку, поэтому раскладывается
    // по позициям пропорционально — отдельной скидки по позиции в модели нет.
    const discountPercent =
        deal.discount === null || deal.discount === undefined
            ? 0
            : Math.min(100, Math.max(0, toNum(deal.discount)))
    const priceFactor = 1 - discountPercent / 100

    for (const item of deal.items || []) {
        const ordered = toNum(item.quantity)
        const gross = toNum(item.amount)
        const amount = gross * priceFactor
        const unitPrice = ordered > 0 ? amount / ordered : 0
        byItem[item.id] = {
            ordered,
            shipped: 0,
            remaining: ordered,
    ...
    for (const sh of deal.shipments || []) {
        ...
        for (const si of sh.items || []) {
            ...
            const row = byItem[si.dealItemId]
            if (!row) continue
            row.shipped += qty
            totalShipped += qty
        }
    ...
    for (const id of Object.keys(byItem)) {
        ...
        row.amountShipped = Math.min(row.amountOrdered, row.shipped * row.unitPrice)
        row.amountRemaining = Math.max(0, row.amountOrdered - row.amountShipped)
        amountShipped += row.amountShipped
    }

    const totalRemaining = Math.max(0, totalOrdered - totalShipped)
    const amountRemaining = Math.max(0, amountOrdered - amountShipped)
    const percent =
        totalOrdered > 0 ? Math.min(100, Math.round((totalShipped / totalOrdered) * 100)) : 0
    const percentAmount =
        amountOrdered > 0 ? Math.min(100, Math.round((amountShipped / amountOrdered) * 100)) : 0
...
/**
 * То же, но сразу по всем позициям сделки — картой { [dealItemId]: остаток }.
 * Нужна формам: в новую отгрузку подставляется остаток от *всех* уже созданных
 * отгрузок (черновиков тоже), иначе позиция из черновика подставилась бы второй
 * раз и сервер отклонил бы сохранение.
 * @param {object} deal — с items и shipments (include items)
 * @param {{ excludeShipmentId?: string|null }} opts — исключить редактируемую отгрузку
 */
export function calculateDealRemainingByItem(deal, { excludeShipmentId = null } = {}) {
    const byItem = {}
    for (const item of deal?.items || []) {
        byItem[item.id] = calculateDealItemRemaining(item, deal?.shipments, {
            excludeShipmentId,
        })
    }
    return byItem
