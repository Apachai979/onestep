...
// Отображаемые имена складов: 1С присылает короткие названия,
// в CRM показываем с уточнением города.
const WAREHOUSE_RENAMES = {
    "Склад ГП": "Склад ГП (Томск)",
    "Склад Елизаровых": "Склад Елизаровых (Томск)",
}
...
export async function syncStockFromOnec() {
    ...
    for (const r of rows) {
        ...
        const rawWarehouse = String(r.warehouse || "").trim() || "Не указан"
        const warehouse = WAREHOUSE_RENAMES[rawWarehouse] || rawWarehouse
