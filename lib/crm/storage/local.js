...
function safeName(name) {
    // Разделители глушим до basename, а не полагаемся на него: имя файла
    // вполне может содержать «/» осмысленной частью — номер КП пишется как
    // «XKJP/1», и basename оставлял от «Коммерческое предложение № XKJP/1
    // от 30.08.2026.pdf» только хвост «1 от 30.08.2026.pdf». В базе имя
    // хранится целиком, так что скачивание не страдало, но найти файл
    // в uploads/ было нельзя. basename ниже остаётся страховкой от обхода
    // каталога — после замены разделителей ему уже нечего отрезать.
    const base = path
        .basename(String(name || "file").replace(/[\\/]+/g, "_"))
