...
export function buildTasksReport({
    ...
    for (const task of tasks) {
        ...
        if (inClosed) {
            ...
            // «чем менеджер занимался», а план на период этого не показывает.
            const t = bump(m.typesMap, task.type, () => ({
                key: task.type,
                closed: 0,
                done: 0,
                failed: 0,
    ...
    const managerRows = Array.from(managers.values())
        .map(m => {
            ...
            return {
                ...
                types: Array.from(m.typesMap.values()).sort((a, b) => b.closed - a.closed),
