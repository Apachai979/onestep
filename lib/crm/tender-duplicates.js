...
import { crmDayStart, crmYmd } from "./datetime"
...
// Поля, описывающие саму процедуру. Когда прежняя процедура уже отыграна,
// они переезжают на новую закупку целиком: карточка обязана описывать ту
// закупку, в которой мы участвуем сейчас, а не ту, что закончилась.
const PROCEDURE_FIELDS = ["purchaseNumber", "auctionUrl", "bidsDeadlineAt", "auctionAt"]

/**
 * Отыграна ли процедура, которую сейчас описывает сделка.
 *
 * Признак — закрытый приём заявок: у запроса цен и котировок другой даты нет
 * вовсе (biddingDate Тендерлэнд отдаёт только по аукционам), а у аукциона к
 * этому моменту закупка тоже уже сыграна или вот-вот сыграет.
 *
 * Граница — начало сегодняшних суток в зоне CRM, как и во вкладке
 * «Просрочены»: по закупке с приёмом заявок «сегодня» заявку ещё подают, и её
 * даты перебивать рано.
 */
function dealProcedureFinished(deal, now) {
    if (!deal.bidsDeadlineAt) return false
    const edge = crmDayStart(crmYmd(now))
    return new Date(deal.bidsDeadlineAt).getTime() < edge.getTime()
}
 ...
 */
export async function linkTenderToDeal(
    tx,
    { tender, deal, auctionCustomerId, userId, now = new Date() },
) {
    // Процедура в карточке отыграна — новая закупка её замещает. Так к сделке
    // по несостоявшемуся запросу цен привязывается объявленный следом
    // электронный аукцион: без замены в карточке остались бы номер и срок
    // закончившейся процедуры, а новый дедлайн не попал бы никуда.
    const replaces = dealProcedureFinished(deal, now)
    ...
    for (const [dealField, tenderField] of Object.entries(FILL_FROM_TENDER)) {
        ...
        if (isBlank(dealField, deal[dealField])) {
            data[dealField] = value
            continue
        }
        if (!replaces || !PROCEDURE_FIELDS.includes(dealField)) continue
        // Дату двигаем только вперёд: закупку заводят и задним числом, и
        // отматывать ею живой срок назад незачем. НМЦК в замену не входит —
        // цену менеджер ставит сам, и она не про то, какая идёт процедура.
        if (value instanceof Date && new Date(deal[dealField]).getTime() >= value.getTime()) {
            continue
        }
        data[dealField] = value
        replaced.push(dealField)
    }
    ...
    const label = key => FILL_LABELS[key] || key
    return {
        // Заполненное и заменённое разводим: первое менеджера не удивит, а
        // второе он должен увидеть — в карточке поменялись номер и сроки.
        filled: Object.keys(data)
            .filter(key => !replaced.includes(key))
            .map(label),
        replaced: replaced.map(label),
    }
}
