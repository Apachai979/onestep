 ...
 *
 * Идентификаторы монотонно растут по мере добавления закупок в Тендерлэнд, и
 * инкремент строится именно на них (searchAfterId). Сравнивать их как строки
 * нельзя: "TL999" оказалось бы больше "TL1000".
 */
...
/** Самый большой идентификатор из списка — точка следующей синхронизации. */
export function maxTenderId(ids) {
    let best = null
    let bestNum = -1
    for (const id of ids) {
        const num = tenderIdNumber(id)
        if (num > bestNum) {
            bestNum = num
            best = id
        }
