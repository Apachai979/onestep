 ...
 * данных (по умолчанию 300 в сутки, 9000 в месяц). Export/Create только считает
 * записи и данные не тратит — поэтому «сколько там всего» можно спрашивать
 * свободно, а вот читать результат нужно с оглядкой на MAX_RECORDS_PER_SYNC.
 */

const BASE_URL = "https://tenderland.ru/Api/v1"

// Сколько закупок разрешаем утащить за одну синхронизацию. Воронка даёт ~20 в
// сутки, так что запас четырёхкратный; ограничение защищает от случая, когда
// фильтр в кабинете расширили и он начал отдавать тысячи записей.
 ...
 *
 * `since` (Date | null) ограничивает выборку датой появления закупки в
 * Тендерлэнде — при обычной синхронизации передаём время прошлой удачной,
 * чтобы не перечитывать уже забранное.
 */
export async function createExport({ since = null, limit = MAX_RECORDS_PER_SYNC } = {}) {
    ...
    const data = await request(
        "Export/Create",
        {
            ...
            searchAfterCreateDate: since ? since.toISOString().slice(0, 19) : undefined,
            limit,
 ...
 */
export async function fetchTenders({ since = null, limit = MAX_RECORDS_PER_SYNC } = {}) {
    const { apiKey } = readConfig()
    const { exportId, totalCount } = await createExport({ since, limit })
    ...
    return {
        rows,
        totalCount,
        // TotalCount у них — это не «сколько нашлось всего», а «сколько попало в
        // выгрузку»: с limit=25 он вернёт ровно 25, даже если закупок сотни.
        // Поэтому упёрлись в потолок — единственный доступный признак того, что
        // остались незабранные; остаток догрузится следующим запуском.
 ...
 */
export async function countTenders({ since = null } = {}) {
    const { totalCount } = await createExport({ since, limit: 100000 })
    return totalCount
}
