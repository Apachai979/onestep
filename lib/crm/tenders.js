...
// Сколько дней неразобранная закупка с закрытым приёмом заявок ещё висит на
// виду (вкладка «Просрочены»), прежде чем уйти в «Мимо». Срок тот же, что у
// отслеживания: две недели — это разбор на ближайшей планёрке, а не свалка.
export const TENDER_EXPIRE_SKIP_DAYS = TENDER_TRACK_DAYS_AFTER_AUCTION

// Причина автоматического отказа. Константа, а не свободный текст: по ней
// список отличает решение машины от решения коллеги.
export const TENDER_EXPIRED_SKIP_REASON = "Срок подачи заявок прошёл, закупка не разобрана"
...
export async function importTenderByNumber({ query, tenderlandId = null } = {}) {
    ...
    const existing = await prisma.tender.findMany({
        ...
        select: {
            ...
            decision: true,
            dealId: true,
            // По сроку список понимает, на какой вкладке искать найденную
            // закупку: у неразобранной с закрытым приёмом заявок это
            // «Просрочены».
            endDate: true,
        },
    })
...
/**
 * Закрывает неразобранные закупки, у которых приём заявок прошёл давно.
 *
 * Заявку по такой закупке подать уже нельзя, а копятся они быстрее, чем
 * разбираются: автопоиск приносит около сотни строк в неделю, и через полгода
 * невыбранный хвост вытеснил бы из выдачи то, что горит (список отдаёт первые
 * 100 строк по возрастанию срока — истёкшие стоят как раз сверху). Первые
 * TENDER_EXPIRE_SKIP_DAYS дней такая закупка видна на отдельной вкладке
 * «Просрочены» — на случай, если заявку подали, а в CRM отметить забыли.
 *
 * Автором решения остаётся никто (decisionById), причина — константа: по ней
 * в списке видно, что закупку закрыла машина. В журнал изменений не пишем —
 * решения по закупкам туда не попадают и от менеджера.
 *
 * Закупку без срока не трогаем: отсчитывать не от чего, и она остаётся в
 * разборе, как и в isTenderTracked. Не трогаем и ту, что менеджер вернул в
 * разбор руками (у неё проставлен decisionAt): раз вернул — значит, знает про
 * срок, и закрывать её обратно той же ночью нельзя.
 */
export async function autoSkipExpiredTenders({ now = new Date() } = {}) {
    const cutoff = new Date(now.getTime() - TENDER_EXPIRE_SKIP_DAYS * DAY_MS)
    const { count } = await prisma.tender.updateMany({
        where: {
            decision: "NEW",
            decisionAt: null,
        ...
        data: {
            decision: "SKIPPED",
            skipReason: TENDER_EXPIRED_SKIP_REASON,
            decisionAt: now,
            decisionById: null,
        },
    })
